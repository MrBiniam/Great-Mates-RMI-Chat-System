...
public class ChatServer extends UnicastRemoteObject implements ChatServerIF {
	...
	public void sendPM(int[] privateGroup, String privateMessage) throws RemoteException{
		// Get the sender's name from the message format (like "John: Hello")
		String senderName = "";
		int colonPos = privateMessage.indexOf(":");
		if (colonPos > 0) {
			senderName = privateMessage.substring(0, colonPos).trim();
			privateMessage = privateMessage.substring(colonPos + 1).trim();
		} else {
			// If colon not found, use a default sender name
			senderName = "Unknown";
		}
		
		// Make sure the sender name is not empty
		if (senderName.isEmpty()) {
			// Try to find the sender from the chatters list based on the sender's index
			for (Chatter c : chatters) {
				if (c.getClient().toString().equals(privateMessage)) {
					senderName = c.getName();
					break;
				}
			}
		...
		// Format as a private message with the PM prefix
		String formattedPrivateMessage = "[PM from " + senderName + "]: " + privateMessage;
		
		// Send to each selected recipient
		Chatter pc;
		for(int i : privateGroup){
			pc = chatters.elementAt(i);
			pc.getClient().messageFromServer(formattedPrivateMessage);
		}
	}
